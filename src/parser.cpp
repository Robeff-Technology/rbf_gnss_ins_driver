...
constexpr std::size_t MAX_NMEA_SENTENCE_LEN = 256;
...
{
  int64_t gps_time = (header.ref_week_num * 7 * 24 * 60 * 60) * second_to_nanosecond +
...
void GnssStreamParser::binary_reset(bool reset_stream_type)
{
  state_ = ParserState::WAIT_SYNC_1;
  index_ = 0;
  expected_length_ = 0;
  header_length_ = 0;
  if (reset_stream_type) {
    stream_type_ = StreamType::UNKNOWN;
  }
}
void GnssStreamParser::nmea_reset(bool reset_stream_type)
{
  nmea_state_ = NmeaState::WAIT_DOLLAR;
  nmea_buffer_.clear();
  nmea_calc_crc_ = 0;
  nmea_recv_crc_ = 0;
  if (reset_stream_type) {
...
void GnssStreamParser::binary_parser(uint8_t byte)
{
  switch (state_) {
    ...
    case ParserState::READ_HEADER:
      ...
      if (index_ >= header_length_) {
        ...
        expected_length_ = header_.header_length + header_.msg_len + sizeof(uint32_t);
    ...
    case ParserState::READ_PAYLOAD_AND_CRC:
      ...
      buffer_[index_++] = byte;
      if (index_ >= expected_length_ && expected_length_ != 0) {
        handle_complete_binary_frame();
        binary_reset(true);
      }
      break;
  }
}
void GnssStreamParser::handle_complete_binary_frame()
{
  const std::size_t crc_offset = header_.header_length + header_.msg_len;
  if (crc_offset + sizeof(uint32_t) > buffer_.size()) {
    stats_.buffer_overflows++;
    return;
  }
  uint32_t received_crc{};
  std::memcpy(&received_crc, buffer_.data() + crc_offset, sizeof(uint32_t));
  const uint32_t calculated_crc =
    calculate_crc32(buffer_.data(), static_cast<int>(header_.header_length + header_.msg_len));
  ...
  if (binary_callback_ != nullptr) {
    ...
    binary_callback_(buffer_.data() + header_.header_length, msg_id);
  }
}
void GnssStreamParser::nmea_parser(uint8_t byte)
{
  switch (nmea_state_) {
    // --------------------------------------------------
    case NmeaState::WAIT_DOLLAR:
      if (byte == '$') {
        nmea_buffer_.clear();
        nmea_calc_crc_ = 0;
        nmea_recv_crc_ = 0;
        nmea_state_ = NmeaState::READ_PREFIX_1;
      }
      break;

    // --------------------------------------------------
    case NmeaState::READ_PREFIX_1:
      // NMEA konuşmacı ID ilk harfi: G (GP, GN, GL, GA, GB)
      if (byte != 'G') {
        nmea_reset(true);
        break;
      }
      ...
      nmea_state_ = NmeaState::READ_PREFIX_2;
      break;

    // --------------------------------------------------
    case NmeaState::READ_PREFIX_2:
      // İkinci harf kontrolü
      if (byte != 'P' && byte != 'N' && byte != 'L' && byte != 'A' && byte != 'B') {
        nmea_reset(true);
        break;
      }
      nmea_prefix_[1] = byte;
      nmea_calc_crc_ ^= byte;
      nmea_buffer_.push_back(static_cast<char>(byte));
      nmea_state_ = NmeaState::READ_BODY;
      break;

    // --------------------------------------------------
    case NmeaState::READ_BODY:
      if (byte == '*') {
        nmea_state_ = NmeaState::READ_CHECKSUM_1;
      } else {
        if (nmea_buffer_.size() >= MAX_NMEA_SENTENCE_LEN) {
          stats_.nmea_overflows++;
          nmea_reset(true);
          break;
        }
        nmea_calc_crc_ ^= byte;
    ...
    // --------------------------------------------------
    case NmeaState::READ_CHECKSUM_1:
      if (!is_hex_char(byte)) {
        stats_.nmea_checksum_errors++;
        nmea_reset(true);
        break;
      }
      nmea_recv_crc_ = static_cast<uint8_t>(hex_to_nibble(byte) << 4);
      nmea_buffer_.push_back(static_cast<char>(byte));
      nmea_state_ = NmeaState::READ_CHECKSUM_2;
      break;

    // --------------------------------------------------
    case NmeaState::READ_CHECKSUM_2:
      if (!is_hex_char(byte)) {
        stats_.nmea_checksum_errors++;
        nmea_reset(true);
        break;
      }
      nmea_recv_crc_ |= hex_to_nibble(byte);
      nmea_buffer_.push_back(static_cast<char>(byte));
      nmea_state_ = NmeaState::WAIT_CR;
      break;

    // --------------------------------------------------
    case NmeaState::WAIT_CR:
      if (byte == '\r') {
        nmea_state_ = NmeaState::WAIT_LF;
      } else {
        stats_.nmea_checksum_errors++;
        nmea_reset(true);
      }
    ...
    // --------------------------------------------------
    case NmeaState::WAIT_LF:
      if (byte == '\n') {
        if (nmea_recv_crc_ == nmea_calc_crc_) {
          if (nmea_callback_ != nullptr) {
...
bool GnssStreamParser::detect_stream_type_and_consume(uint8_t byte)
{
  if (byte == '$') {
    stream_type_ = StreamType::NMEA;
    nmea_reset(false);
    nmea_parser(byte);
    return true;
  }
  if (byte == SYNC1) {
    stream_type_ = StreamType::BINARY;
    binary_reset(false);
    binary_parser(byte);
    return true;
  }
...
{
  if (stream_type_ == StreamType::UNKNOWN) {
    if (detect_stream_type_and_consume(byte)) {
      return;
    }
    return;
  }
  if (stream_type_ == StreamType::BINARY) {
    binary_parser(byte);
    return;
  }

  if (stream_type_ == StreamType::NMEA) {
    nmea_parser(byte);
    return;
  }
}
