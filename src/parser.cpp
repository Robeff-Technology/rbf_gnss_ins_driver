...
namespace rbf_gnss_ins_driver
{
  ...
  void GnssStreamParser::check_binary_message(const uint8_t *buffer,
                                              size_t size)
  {
    ...
    uint32_t crc;
    memcpy(reinterpret_cast<uint8_t *>(&header_), buffer, sizeof(Header));
    memcpy(reinterpret_cast<uint8_t *>(&crc),
           buffer + header_.msg_len + header_.header_length, sizeof(uint32_t));
    if (calculate_crc32(buffer, size - 4) == crc)
    {
      if (binary_callback_ != nullptr)
      {
        auto msg_id = static_cast<GnssStreamParser::MessageId>(header_.msg_id);
        binary_callback_(buffer + header_.header_length, msg_id);
      }
    }
  }
  ...
  {
    constexpr size_t MAX_HEADER_LEN = 28;
    constexpr size_t MAX_PAYLOAD_LEN = 128;

    static std::deque<uint8_t> sync_window; // For sliding sync search


    if (mode_ == ParserMode::UNKNOWN)
    {
      if (byte == '$')
      {
        nmea_buffer_.clear();
        nmea_buffer_ += byte;
        nmea_started_ = true;
        mode_ = ParserMode::NMEA;
        return;
      }
      sync_window.push_back(byte);
      if (sync_window.size() > 4) // 3 sync bytes + 1 header_len
        sync_window.pop_front();

      if (sync_window.size() == 4 &&
          sync_window[0] == first_synch &&
          sync_window[1] == second_synch &&
          sync_window[2] == third_synch)
      {
        if (sync_window[3]> MAX_HEADER_LEN)
        {
          binary_buffer_.clear();
          binary_expected_length_ = 0;
          binary_header_found_ = false;
          mode_ = ParserMode::UNKNOWN;
          return;
        }
        else
        {
          // Found a valid binary sync
          binary_buffer_.clear();
          for (auto b : sync_window)
            binary_buffer_.push_back(b);

          mode_ = ParserMode::BINARY;
          binary_expected_length_ = sync_window[3]; // initial header_len
        }
      }
    ...
    if (mode_ == ParserMode::NMEA)
    {
      nmea_buffer_ += byte;
      if (byte == '\n')
      { // or '\r' or both depending on GNSS device
        if (verify_nmea_checksum(nmea_buffer_))
        {
    ...
    if (mode_ == ParserMode::BINARY)
    {
      binary_buffer_.push_back(byte);
      // After header, estimate total message length
      if (binary_buffer_.size() == binary_expected_length_ &&
          binary_header_found_ == false)
      {
        uint16_t msg_len = binary_buffer_[8] | (binary_buffer_[9] << 8);
        ...
        {
          binary_expected_length_ += msg_len + 4; // payload + CRC
          binary_header_found_ = true;
        }
      }

      if (binary_header_found_ &&
          binary_buffer_.size() >= binary_expected_length_)
      {
        check_binary_message(binary_buffer_.data(), binary_expected_length_);
        binary_buffer_.clear();
        binary_expected_length_ = 0;
        mode_ = ParserMode::UNKNOWN;
        binary_header_found_ = false;
