...
void GnssStreamParser::handle_byte(uint8_t byte) {
  ...
  if (mode_ == ParserMode::BINARY) {
    ...
    // Check if we can read header length
    if (binary_buffer_.size() == 4) {

      if(binary_buffer_[0] != first_synch || 
         binary_buffer_[1] != second_synch ||
         binary_buffer_[2] != third_synch) {
        binary_buffer_.clear();
        mode_ = ParserMode::UNKNOWN;
        return;      
      }
      size_t header_len = binary_buffer_[3];
      binary_expected_length_ = header_len;
    }
    ...
    if (binary_expected_length_ > 0 &&
        binary_buffer_.size() >= binary_expected_length_) {
      check_binary_message(binary_buffer_.data(), binary_buffer_.size());
      binary_buffer_.clear();
      binary_buffer_.size();
