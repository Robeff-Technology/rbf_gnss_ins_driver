...
{
GnssInsDriver::GnssInsDriver(const rclcpp::NodeOptions & options)
: Node("rbf_gnss_ins_driver", options)
{
  load_parameters();
  init_publishers();
  try {
  ...
  converter_ = std::make_shared<Converter>(
    config_params_.use_ros_time_,
    static_cast<Converter::AltitudeMode>(config_params_.altitude_mode_));
  gnss_parser_ = std::make_shared<GnssStreamParser>(
    std::bind(&GnssInsDriver::binary_callback, this, std::placeholders::_1, std::placeholders::_2),
    std::bind(&GnssInsDriver::nmea_callback, this, std::placeholders::_1));
  timer_ = this->create_wall_timer(
    std::chrono::milliseconds(1000 / config_params_.working_frequency_),
    std::bind(&GnssInsDriver::timer_callback, this));
}
...
void GnssInsDriver::load_parameters()
{
  ...
  config_params_.serial_.serial_port_ =
    this->declare_parameter("serial_config.port", "/dev/ttyUSB0");
  ...
  config_params_.topics_.nav_sat_fix_topic_ =
    this->declare_parameter("topic_config.nav_sat_fix_topic", "nav_sat_fix");
  config_params_.topics_.twist_topic_ =
    this->declare_parameter("topic_config.twist_topic", "twist");
  config_params_.topics_.temperature_topic_ =
    this->declare_parameter("topic_config.temperature_topic", "temperature");
  ...
  config_params_.frames_.odometry_frame_ =
    this->declare_parameter("odometry_config.odometry_frame", "odometry");
  config_params_.topics_.odometry_topic_ =
    this->declare_parameter("odometry_config.odometry_topic", "odometry");
  config_params_.odometry_.use_odometry_ =
    this->declare_parameter("odometry_config.use_odometry", false);
  ...
  RCLCPP_INFO(
    this->get_logger(), "nav_sat_fix_topic: %s", config_params_.topics_.nav_sat_fix_topic_.c_str());
  RCLCPP_INFO(this->get_logger(), "twist_topic: %s", config_params_.topics_.twist_topic_.c_str());
  RCLCPP_INFO(
    this->get_logger(), "temperature_topic: %s", config_params_.topics_.temperature_topic_.c_str());
  ...
  RCLCPP_INFO(
    this->get_logger(), "odometry_frame: %s", config_params_.frames_.odometry_frame_.c_str());
  RCLCPP_INFO(
    this->get_logger(), "odometry_topic: %s", config_params_.topics_.odometry_topic_.c_str());
...
void GnssInsDriver::init_publishers()
{
  ...
  pub_heading_ =
    this->create_publisher<rbf_gnss_ins_driver::msg::Heading>("/robins/raw/heading", qos);
  pub_ecef_ = this->create_publisher<rbf_gnss_ins_driver::msg::ECEF>("/robins/raw/ecef", qos);
  pub_imu_status_ =
    this->create_publisher<rbf_gnss_ins_driver::msg::ImuStatus>("/robins/status/imu_status", qos);
  pub_gnss_vel_ =
    this->create_publisher<rbf_gnss_ins_driver::msg::GnssVel>("/robins/raw/gnss_vel", qos);
  pub_gnss_status_ =
    this->create_publisher<rbf_gnss_ins_driver::msg::GnssStatus>("/robins/status/gnss_status", qos);
  pub_rtcm_status_ =
    this->create_publisher<rbf_gnss_ins_driver::msg::RTCMStatus>("/robins/status/rtcm_status", qos);
  ...
  pub_temperature_ =
    this->create_publisher<sensor_msgs::msg::Temperature>("/robins/raw/temperature", qos);
  pub_ecef_twist_ = this->create_publisher<geometry_msgs::msg::TwistWithCovarianceStamped>(
    "/robins/raw/ecef_twist", qos);
  ...
  if (config_params_.odometry_.use_odometry_) {
    pub_odometry_ =
      this->create_publisher<nav_msgs::msg::Odometry>(config_params_.topics_.odometry_topic_, qos);
    tf_broadcaster_ = std::make_shared<tf2_ros::TransformBroadcaster>(this);
    tf_static_broadcast_ = std::make_shared<tf2_ros::StaticTransformBroadcaster>(this);
    ll_to_utm_transform_ = std::make_shared<LlToUtmTransform>(
      config_params_.odometry_.lat_origin_, config_params_.odometry_.long_origin_,
      config_params_.odometry_.alt_origin_);
  ...
  sub_rtcm_ = this->create_subscription<mavros_msgs::msg::RTCM>(
    config_params_.topics_.rtcm_topic_, qos,
    std::bind(&GnssInsDriver::rtcm_callback, this, std::placeholders::_1));
}
...
void GnssInsDriver::binary_callback(const uint8_t * data, GnssStreamParser::MessageId id)
{
  ...
  if (GnssStreamParser::MessageId::ECEF == id) {
    ...
    auto ecef_twist_msg =
      converter_->ecef_to_twist_msg(ecef_, raw_imux_, config_params_.frames_.gnss_frame_);
    ...
    auto imu_status_msg =
      converter_->raw_imu_to_imu_status(raw_imux_, config_params_.frames_.imu_frame_);
    ...
    auto temperature_msg =
      converter_->raw_imu_to_temperature_msg(raw_imux_, config_params_.frames_.imu_frame_);
    ...
    GnssStreamParser::MessageId::GNSSPOS == id || GnssStreamParser::MessageId::GNSSPOS_1 == id) {
    gnss_pos_ = *reinterpret_cast<const BestGnssPos *>(data);
    auto gnss_status_msg =
      converter_->gnss_pos_to_gnss_status_msg(gnss_pos_, config_params_.frames_.gnss_frame_);
    pub_gnss_status_->publish(gnss_status_msg);

    auto nav_sat_fix_msg =
      converter_->gnss_pos_to_nav_sat_fix_msg(gnss_pos_, config_params_.frames_.gnss_frame_);
    pub_nav_sat_fix_raw_->publish(nav_sat_fix_msg);
  } else if (
    GnssStreamParser::MessageId::GNSSVEL == id || GnssStreamParser::MessageId::GNSSVEL_1 == id) {
    ...
    if (Converter::is_ins_active(ins_pva_.ins_status)) {
      ...
      if (config_params_.odometry_.use_odometry_) {
        double x, y, z;
        ll_to_utm_transform_->transform(
          ins_pva_.latitude, ins_pva_.longitude, ins_pva_.height, x, y, z);

        auto odometry_msg = converter_->convert_to_odometry_msg(
          ins_pva_, raw_imux_, x, y, z, config_params_.frames_.odometry_frame_);
        pub_odometry_->publish(odometry_msg);

        auto transform = converter_->create_transform(
          odometry_msg.pose.pose, config_params_.frames_.odometry_frame_);
      ...
      auto twist_msg =
        converter_->ins_to_twist_msg(ins_pva_, raw_imux_, config_params_.frames_.gnss_frame_);
...
{
  // Eğer shutdown tetiklendiyse artık okuma yapma
  ...
  try {
    const int len = serial_port_ptr_->read(reinterpret_cast<char *>(buffer), sizeof(buffer));
    ...
    RCLCPP_WARN(
      get_logger(), "Serial read error (%d/%d): %s", consecutive_read_errors_,
      MAX_CONSECUTIVE_READ_ERRORS, e.what());

    if (consecutive_read_errors_ >= MAX_CONSECUTIVE_READ_ERRORS) {
      shutdown_triggered_ = true;

      RCLCPP_FATAL(
        get_logger(), "Serial read failed %d times consecutively. Shutting down GNSS driver.",
...
{
  converter_->is_delay_high(gnss_parser_->get_unix_time_ns())
    ? stat.summary(diagnostic_msgs::msg::DiagnosticStatus::ERROR, "HIGH_DELAY")
    : stat.summary(diagnostic_msgs::msg::DiagnosticStatus::OK, "OK");
  stat.add("INS_STATUS", Converter::is_ins_active(ins_pva_.ins_status) ? "ACTIVE" : "INACTIVE");

  const auto & s = gnss_parser_->stats();

  auto add_if_nonzero = [&stat](const std::string & key, uint64_t value) {
    if (value > 0) {
      stat.add(key, value);
    }
  };

  add_if_nonzero("parser.messages_ok", s.messages_ok);
  add_if_nonzero("parser.sync_errors", s.sync_errors);
  add_if_nonzero("parser.header_length_errors", s.header_length_errors);
  add_if_nonzero("parser.payload_length_errors", s.payload_length_errors);
  add_if_nonzero("parser.crc_errors", s.crc_errors);
  add_if_nonzero("parser.buffer_overflows", s.buffer_overflows);
  add_if_nonzero("parser.nmea_overflows", s.nmea_overflows);
  add_if_nonzero("parser.nmea_checksum_errors", s.nmea_checksum_errors);
}
